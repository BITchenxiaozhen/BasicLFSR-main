...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        # self.conv_de = nn.Conv3d(60, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)
        self.macPI = MacPI(angRes)
        self.EPIFilter = EPIFilter(angRes=angRes)
        # self.iAFF = iAFF(60, 6)
    def forward(self, buffer):
        buffer1 = self.spa_trans(self.ang_trans(buffer))
        buffer2 = self.EPIFilter(buffer1)
        buffer3 = self.macPI(buffer2)+buffer
        # buffer11 = buffer1 + buffer2 + buffer3
        # buffer11 = self.conv_de(buffer11)
