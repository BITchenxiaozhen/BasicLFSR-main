...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.conv_de = nn.Conv3d(60*2, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)

        self.EPIFilter = EPIFilter(angRes=angRes)

    def forward(self, buffer):
        buffer1 = self.ang_trans(buffer)
        buffer2 = self.spa_trans(buffer1)
        buffer3 = self.EPIFilter(buffer)
        buffer = self.conv_de(torch.cat([buffer2,buffer3],dim = 1)) + buffer
        return buffer
