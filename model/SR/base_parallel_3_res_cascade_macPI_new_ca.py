...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.SElayer = SELayer(60,6)
        # self.conv_de = nn.Conv3d(60*4, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)
        self.conv_1x1 = nn.Conv3d(60, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False)
        self.conv_2x1 = nn.Conv3d(60, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False)
        self.conv_3x1 = nn.Conv3d(60, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False)

        self.EPIFilter = EPIFilter(angRes=angRes)

    def forward(self, buffer):
        buffer1 = self.EPIFilter(buffer)
        buffer2 = self.spa_trans(self.ang_trans(buffer))
        buffer3 = self.macPI(buffer)
        buffer21 = self.conv_1x1(buffer1-buffer2)+buffer2
        buffer32 = self.conv_2x1(buffer2-buffer3)+buffer3
        buffer111 = self.SElayer(buffer1)
        buffer222 = self.SElayer(buffer21)
