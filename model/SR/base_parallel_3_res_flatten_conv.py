...
class FlattenedConvNet(nn.Module):
    def __init__(self, in_channels, out_channels, lateral_kernel_size, vertical_kernel_size, horizontal_kernel_size):
        super(FlattenedConvNet, self).__init__()

        # self.l_conv = nn.Conv3d(in_channels, out_channels, kernel_size=lateral_kernel_size)
        self.l_conv = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 1, 1), padding=(0, 0, 0))
        self.v_conv = nn.Conv3d(out_channels, out_channels, kernel_size=(3, 1, 1), padding=(1, 0, 0))
        self.h_conv = nn.Conv3d(out_channels, out_channels, kernel_size=(1, 1, 3), padding=(0, 0, 1))

    def forward(self, x):
        # b,c,a,h,w = x.size()
        # x = x.permute(0,2,1,3,4)
        l_output = self.l_conv(x)
        # l_output = l_output.permute(0, 2, 1, 3, 4)
        v_output = self.v_conv(l_output)
        h_output = self.h_conv(v_output)

        # flattened_output = torch.cat((l_output, v_output, h_output), dim=1)
        return h_output
...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.conv_de = FlattenedConvNet(180, 60, (1, 1, 1), (3, 1, 1), (3, 1, 1))
