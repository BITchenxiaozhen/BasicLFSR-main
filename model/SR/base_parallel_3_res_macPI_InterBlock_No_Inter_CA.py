...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        b, c, a, _, _ = x.size()
        y = self.avg_pool(x).view(b, a, c)  # 对应Squeeze操作
...
class AltFilter(nn.Module):
    ...
    def forward(self, buffer):
        ...
        buffer = self.SELayer(buffer).view(b, 5, c, a, h, w)
        return buffer
...
class AltFilter_2(nn.Module):
    ...
    def forward(self, buffer):
        ...
        buffer = torch.cat([buffer1,buffer2,buffer3,buffer4, buffer5],dim = 1)
        buffer = self.SELayer(buffer).view(b, g, c, a, h, w)
        return buffer
