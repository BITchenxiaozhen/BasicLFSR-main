...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.conv_de = nn.Conv3d(60*3, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False)
    ...
    def forward(self, buffer):
        ...
        buffer11 = self.conv_de(buffer_tmp)
        buffer11 = self.SELayer(buffer11)
...
class Partial_conv3(nn.Module):
    ...
    def __init__(self, dim, n_div, forward="split_cat"):
        ...
        self.partial_conv3_1 = nn.Conv3d(dim, self.dim_conv3, 1, 1, 0, bias=False)
        self.partial_conv3_2 = nn.Conv3d(dim, self.dim_conv3, 1, 1, 0, bias=False)
    ...
    def forward_slicing(self, x, y):
        # only for inference
        x = x.clone()   # !!! Keep the original input intact for the residual connection later
        y = y.clone()
        y[:, :self.dim_conv3, :, :] = self.partial_conv3(x[:, :self.dim_conv3, :, :])

        return y

    def forward_split_cat(self, x, y):
        # for training/inference
        x1, x2 = torch.split(x, [self.dim_conv3, self.dim_untouched], dim=1)
        y1, y2 = torch.split(y, [self.dim_conv3, self.dim_untouched], dim=1)

        x = torch.cat((x1, y2), 1)
        y = torch.cat((y1, x2), 1)

        x = self.partial_conv3_1(x)
        y = self.partial_conv3_2(y)
