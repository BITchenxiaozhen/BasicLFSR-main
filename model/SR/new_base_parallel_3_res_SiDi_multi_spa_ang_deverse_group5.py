...
class SiDi(nn.Module):
    '''
    多特征融合 iAFF
    '''

    def __init__(self, channels=64, r=4):
        super(SiDi, self).__init__()
        inter_channels = int(channels // r)
        self.local_att_si = nn.Sequential(
        ...
        self.local_att_dix = nn.Sequential(
            nn.Conv3d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
            # nn.BatchNorm3d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(inter_channels, channels, kernel_size=1, stride=1, padding=0),
        ...
        self.local_att_diy = nn.Sequential(
            nn.Conv3d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
            # nn.BatchNorm3d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(inter_channels, channels, kernel_size=1, stride=1, padding=0),
        ...
        self.local_att_diz = nn.Sequential(
            nn.Conv3d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
            # nn.BatchNorm3d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(inter_channels, channels, kernel_size=1, stride=1, padding=0),
    ...
    def forward(self, x,y,z, residual):
        ...
        disi_attn = self.local_att_si(residual)
        dix_attn = self.local_att_dix(x + residual)
        diy_attn = self.local_att_diy(y + residual)
        diz_attn = self.local_att_diz(z + residual)

        out = residual + disi_attn * si + dix_attn * dix + diy_attn * diy + diz_attn * diz
...
class AltFilter(nn.Module):
    ...
    def forward(self, buffer):
        ...
        # buffer11 = self.iAFF1(buffer3,buffer2)
        # buffer111 = self.iAFF2(buffer11,buffer1)
        # buffer1111 = self.iAFF3(buffer111,buffer)
        buffer1111 = self.sidi(buffer1,buffer2,buffer3, buffer)
        return buffer1111
