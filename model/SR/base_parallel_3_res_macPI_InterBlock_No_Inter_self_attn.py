...
class InterBlock(nn.Module):
    ...
    def forward(self,x,x2):
        x = rearrange(x, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=self.angRes, a2=self.angRes)
        x2 = rearrange(x2, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=self.angRes, a2=self.angRes)
        ...
        # out_a = self.Ang2Spa_2(out_a)

        out_a = rearrange(buffer_spa1, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        out_s = rearrange(buffer_spa2, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        return out_a, out_s
...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        #     nn.Conv3d(300, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        # )
        self.conv_key = nn.Sequential(
            nn.Conv3d(300, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        )
        # self.conv_recov = nn.Sequential(
        #     nn.Conv3d(60, 300, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        # )
        # self.conv_de = nn.Conv3d(60*5, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)

        self.EPIFilter = EPIFilter(angRes=angRes)

        self.InterBlock = InterBlock(angRes=angRes, channels=60)

    def cal_attention(self, query, key):
        b, c, a, h, w = query.shape
        # query_conv = self.conv_query(query)
        query_conv = query
        query_temp = rearrange(query_conv, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=self.angRes, a2=self.angRes)
        query = query_temp.view(b, -1, h * self.angRes * w * self.angRes).permute(0, 1, 2)  # B * C//8 * (H * W)

        key_conv = self.conv_key(key)
        key_temp = rearrange(key_conv, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=self.angRes, a2=self.angRes)
        key = key_temp.view(b, -1, h * self.angRes * w * self.angRes).permute(0, 2, 1)  # B * (H * W)* C//8

        attention = torch.bmm(query, key)  # B * (H * W) * (H * W)
        attention = self.softmax_(attention).permute(0, 2, 1)

        self_attention_map = torch.bmm(key, attention).permute(0, 2, 1)
        self_attention_map = self_attention_map.view(b, -1, h * self.angRes, w * self.angRes)  # B * C * H * W
        out = self.gamma * self_attention_map + query_temp
        out = rearrange(out, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        out = out.view(b, 5, -1, a, h, w)
        return out
    ...
    def forward(self, buffer):
        ...
        buffer = self.cal_attention(buffer, buffer)
        return buffer

class AltFilter_2(nn.Module):
    def __init__(self, angRes):
        super(AltFilter_2, self).__init__()
        self.angRes = angRes
        self.spa_trans = CSWinIR(is_spa=True, transtype=1, angRes=angRes)
        self.ang_trans = CSWinIR(is_spa=False, transtype=2, angRes=angRes)
        self.softmax_ = nn.Softmax(dim=2)
    ...
    def forward(self, buffer):
        ...
        buffer1 = torch.squeeze(buffer[:, 0, :, :, :, :],1)
        buffer2 = torch.squeeze(buffer[:, 1, :, :, :, :],1)
        buffer3 = torch.squeeze(buffer[:, 2, :, :, :, :],1)
        buffer4 = torch.squeeze(buffer[:, 3, :, :, :, :],1)
        buffer5 = torch.squeeze(buffer[:, 4, :, :, :, :],1)

        buffer1 = self.ang_trans(buffer1)
        buffer2 = self.spa_trans(buffer2)
        buffer3 = self.EPIFilter(buffer3)
        buffer4, buffer5 = self.InterBlock(buffer4, buffer5)
        buffer = torch.cat([buffer1,buffer2,buffer3,buffer4, buffer5],dim = 1)
        buffer = self.cal_attention(buffer, buffer)
        return buffer
...
class body(nn.Module):
    def __init__(self, angRes):
        ...
        self.AltFilter2 = AltFilter_2(angRes=angRes)
        self.AltFilter3 = AltFilter_2(angRes=angRes)
        self.AltFilter4 = AltFilter_2(angRes=angRes)
        self.AltFilter5 = AltFilter_2(angRes=angRes)
        self.AltFilter6 = AltFilter_2(angRes=angRes)
        self.AltFilter7 = AltFilter_2(angRes=angRes)
        self.AltFilter8 = AltFilter_2(angRes=angRes)
        self.EPIFilter = EPIFilter(angRes=angRes)
        self.gamma = nn.Parameter(torch.zeros(1))
        # self.conv_de = nn.Conv3d(60 * 5, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)
        self.conv_de = nn.Conv3d(60 * 5, 60, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False)

    def forward(self, buffer):
        batchsize, num_channels, resAng, height, width = buffer.data.size()
        ...
        buffer8 = buffer8.view(batchsize, -1, resAng, height, width)
        buffer8 = self.conv_de(buffer8)
