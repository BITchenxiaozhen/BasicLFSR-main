...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.angRes = angRes
        self.spa_trans = CSWinIR(is_spa=True, transtype=1, angRes=angRes, depths=1)
        self.ang_trans = CSWinIR(is_spa=False, transtype=2, angRes=angRes, depths=1)

        self.softmax_ = nn.Softmax(dim=2)
        self.conv_de = nn.Conv3d(60*4, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)
        self.macPI = nn.Conv2d(60, 25, kernel_size=5, dilation=1, padding =2, stride =5)
        self.conv_up = nn.Conv3d(1, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1)
    ...
    def forward(self, buffer):
        ...
        buffer3 = rearrange(buffer, 'b c (a1 a2) h w -> b c (h a1) (w a2)', a1=self.angRes, a2=self.angRes)
        buffer3 = self.macPI(buffer3).view(b,-1, a, h, w)
        buffer3 = self.conv_up(buffer3)

        buffer4 = self.EPIFilter(buffer)
        buffer = self.conv_de(torch.cat([buffer1,buffer2,buffer3,buffer4],dim = 1)) + buffer
        return buffer
...
class body(nn.Module):
    def __init__(self, angRes):
        ...
        self.AltFilter9 = AltFilter(angRes=angRes)
        self.AltFilter10 = AltFilter(angRes=angRes)
        self.AltFilter11 = AltFilter(angRes=angRes)
        self.AltFilter12 = AltFilter(angRes=angRes)
        self.AltFilter13 = AltFilter(angRes=angRes)
        self.AltFilter14 = AltFilter(angRes=angRes)
        self.AltFilter15 = AltFilter(angRes=angRes)
        self.AltFilter16 = AltFilter(angRes=angRes)
        self.EPIFilter = EPIFilter(angRes=angRes)
    ...
    def forward(self, buffer):
        ...
        buffer9 = self.AltFilter9(buffer8)
        buffer10 = self.AltFilter10(buffer9)
        buffer11 = self.AltFilter11(buffer10)
        buffer12 = self.AltFilter12(buffer11)
        buffer13 = self.AltFilter13(buffer12)
        buffer14 = self.AltFilter14(buffer13)
        buffer15 = self.AltFilter15(buffer14)
        buffer16 = self.AltFilter16(buffer15)

        buffer16 = self.EPIFilter(buffer16)
        return buffer16
