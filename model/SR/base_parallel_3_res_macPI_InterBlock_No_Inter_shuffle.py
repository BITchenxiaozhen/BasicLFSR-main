...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.conv_query = nn.Sequential(
            nn.Conv3d(60, 6, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        )
        self.conv_key = nn.Sequential(
            nn.Conv3d(60, 6, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        )
        self.conv_recov = nn.Sequential(
            nn.Conv3d(12, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False),
        )
    ...
    def forward(self, buffer):
        ...
        buffer = channel_shuffle(x = buffer,groups = 5)
        return buffer
...
class body(nn.Module):
    def __init__(self, angRes):
        ...
        self.squeeze = nn.Sequential(
            nn.Conv3d(60*8, 60, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False),
        )
        self.attention = MixtureOfSoftMaxACF(n_mix=1, d_k=60)
        self.conv_query = nn.Sequential(
            nn.Conv3d(60, 6, kernel_size=(1, 1, 1), padding=(0, 0, 0), dilation=1, bias=False),
        )
        self.conv_key = nn.Sequential(
    ...
    def forward(self, buffer):
        ...
        # buffer = torch.cat([buffer1,buffer2,buffer3,buffer4,buffer5,buffer6,buffer7,buffer8],dim=1)
        # buffer = self.squeeze(buffer)
        #
        # b, c, a, h, w = buffer.shape
        # # buffer = self.conv_query(buffer)
        # query_temp = rearrange(buffer, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=5, a2=5)
        # query = query_temp.view(b, -1, h * a * w )  # B * C//8 * (H * W)
        # #
        # # buffer9 = self.conv_key(buffer9)
        # key_temp = rearrange(buffer9, 'b c (a1 a2) h w -> b c (a1 h) (a2 w) ', a1=5, a2=5)
        # key = key_temp.view(b, -1, h * a * w)  # B * (H *
        #
        # buffer,attn = self.attention(query,key,key)
        #
        # self_attention_map = buffer.permute(0, 2, 1).view(b, -1, h * 5, w * 5)  # B * C * H * W
        # out = self.gamma * self_attention_map
        # buffer = rearrange(out, 'b c (a1 h) (a2 w) -> b c (a1 a2) h w', a1=5, a2=5)
...
class MixtureOfSoftMaxACF(nn.Module):
    ...
    def __init__(self, n_mix, d_k, attn_dropout=0.1):
        ...
        self.temperature = np.power(8, 0.5)
        self.n_mix = n_mix
