...
class ODConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 reduction=0.0625, kernel_num=4, transtype=1, angRes=5):
    ...
    def check_image_size(self, x, transtype):
        if transtype == 1:
            ...
            # x = F.pad(x, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        return x
    def un_check_image_size(self, x, transtype):

        if transtype == 1:
            x = rearrange(x, '(b a1 a2) c h w-> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes, h=self.h, w=self.w)
        if transtype == 2:
            x = rearrange(x, '(b h w) c a1 a2->b c (a1 a2) h w', a1=self.angRes, a2=self.angRes, h=self.h, w=self.w)
        if transtype == 3:
            x = rearrange(x, '(b h a1) c w a2->b c (a1 a2) h w', a1=self.angRes, a2=self.angRes, h=self.h, w=self.w)
        if transtype == 4:
            x = rearrange(x, '(b w a2) c h a1->b c (a1 a2) h w', a1=self.angRes, a2=self.angRes, h=self.h, w=self.w)
        if transtype == 5:
            pass
            # x = rearrange(x, 'b c (h a1) (w a2)->b (a1 a2) c h w', a1=self.angRes, a2=self.angRes, h=self.h, w=self.w)
        return x
    def forward(self, x):

        x = self.check_image_size(x, transtype=self.transtype)
        x = self._forward_impl(x)
        x = self.un_check_image_size(x, transtype=self.transtype)
        return x
...
class MacPI(nn.Module):
    def __init__(self,angRes):
        ...
        self.macPI_ang1 = nn.Sequential(
            ODConv2d(60, 25, kernel_size=1, dilation=1, padding=0, stride=1, reduction=0.0625, kernel_num=4, transtype=5),
            ODConv2d(25, 25, kernel_size=5, dilation=1, padding=2, stride=5, groups=25, reduction=0.0625, kernel_num=4, transtype=5),
            nn.ReLU()
        )
        ...
        self.macPI_ang2 = nn.Sequential(
            ODConv2d(60, 25, kernel_size=1, dilation=1, padding=0, stride=1, reduction=0.0625, kernel_num=4, transtype=5),
            ODConv2d(25, 25, kernel_size=5 * 2, dilation=1, padding=3, stride=5, groups=25, reduction=0.0625,
                     kernel_num=4, transtype=5),
            nn.ReLU()
        )
...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.spa_trans = nn.Sequential(
            ODConv2d(60, 60, kernel_size=3, dilation=1,padding=1, groups =6, reduction=0.0625,kernel_num=4, transtype=1, angRes=self.angRes),
            ODConv2d(60, 60, kernel_size=3, dilation=1,padding=1, groups =10, reduction=0.0625, kernel_num=4, transtype=1, angRes=self.angRes),
        )
        self.ang_trans = nn.Sequential(
            ODConv2d(60, 60, kernel_size=5, dilation=1,padding=2,groups =6, reduction=0.0625,kernel_num=4, transtype=2, angRes=self.angRes),
            ODConv2d(60, 60, kernel_size=5, dilation=1,padding=2,groups =10, reduction=0.0625, kernel_num=4, transtype=2, angRes=self.angRes),
        )
        ...
        self.EPIFilter = nn.Sequential(
            ODConv2d(60, 60, kernel_size=5, dilation=1,padding=2, groups =6, reduction=0.0625,kernel_num=4, transtype=3, angRes=self.angRes),
            ODConv2d(60, 60, kernel_size=5, dilation=1,padding=2, groups =10, reduction=0.0625, kernel_num=4, transtype=4, angRes=self.angRes),
        )
        self.iAFF = iAFF(60, 6)

    def forward(self, buffer):
        buffer_tmp = self.ang_trans(buffer)
        buffer1 = self.spa_trans(buffer_tmp)
