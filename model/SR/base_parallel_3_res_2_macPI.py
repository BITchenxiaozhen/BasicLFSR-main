...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.macPI_ang = nn.Conv2d(60, 60, kernel_size=angRes, dilation=1, padding=2, stride =angRes)
        self.macPI_spa = nn.Conv2d(60, 60, kernel_size=3, dilation=angRes, padding=angRes, stride=1)
        self.Ang2Spa = nn.Sequential(
            nn.Conv2d(60, int(angRes*angRes*60), kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def forward(self, buffer):
        ...
        buffer1 = rearrange(buffer1, 'b c (a1 a2) h w -> b c (h a1) (w a2)', a1=self.angRes, a2=self.angRes)
        buffer1 = self.macPI_ang(buffer1)
        buffer1 = self.Ang2Spa(buffer1)
        buffer1 = rearrange(buffer1, 'b c (h a1) (w a2) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)


        buffer2 = self.spa_trans(buffer)
        buffer2 = rearrange(buffer2, 'b c (a1 a2) h w -> b c (h a1) (w a2)', a1=self.angRes, a2=self.angRes)
        buffer2 = self.macPI_spa(buffer2)
        buffer2 = rearrange(buffer2, 'b c (h a1) (w a2) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        buffer = self.EPIFilter(buffer)
