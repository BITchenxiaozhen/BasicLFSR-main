...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        self.macPI_big = nn.Sequential(
            nn.Conv2d(60, 60, kernel_size=31, padding=int(31/2),stride=1,groups=60),
            nn.ReLU(),
            nn.Conv2d(60, 60, kernel_size=31, padding=int(31 / 2), stride=1, groups=60),
            nn.ReLU(),
        )
        self.macPI_small = nn.Sequential(
            nn.Conv2d(60, 60, kernel_size=3, padding=int(3 / 2), stride=1),
            nn.ReLU(),
            nn.Conv2d(60, 60, kernel_size=3, padding=int(3 / 2), stride=1, groups=30),
            nn.ReLU()
        )
    ...
    def forward(self, buffer):
        ...
        buffer_tmp = rearrange(buffer, 'b c (a1 a2) h w -> b c (h a1) (w a2)', a1=self.angRes, a2=self.angRes)
        buffer4 = self.macPI_big(buffer_tmp)
        buffer4 = rearrange(buffer4, 'b c (h a1) (w a2) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)

        buffer5 = self.macPI_small(buffer_tmp)
        buffer5 = rearrange(buffer5, 'b c (h a1) (w a2) -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)

        buffer = self.conv_de(torch.cat([buffer1,buffer2,buffer3,buffer4, buffer5],dim = 1)) + buffer
        return buffer
