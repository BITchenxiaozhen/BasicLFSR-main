...
class AltFilter(nn.Module):
    def __init__(self, angRes):
        ...
        # self.conv_de = self.rev_conv2d(input_channel=180,output_channel=60,kernel_size=3)


        self.EPIFilter = EPIFilter(angRes=angRes)

    def rev_conv2d(self, outs, input_channel, output_channel, kernel_size):
        b, c, _, h, w = outs.size()
        outs = rearrange(outs, 'b c (a1 a2) h w -> b c (a1 h a2 w)', a1=self.angRes, a2=self.angRes)

        # if data_format == 'NHWC':
        #     outs = outs.permute(0, 3, 1, 2).contiguous()

        # pre_shape = [-1] + list(outs.shape[1:])
        # hw_dim = np.prod(outs.shape[2:])
        # new_shape = [-1, outs.shape[1], hw_dim]
        # outs = outs.view(new_shape)

        # num_outs = outs.shape[-1]
        # kernel = (kernel_size, output_channel, 1)
        # conv_layer = nn.Conv1d(input_channel, output_channel, *kernel)
        # kernel = (kernel_size, output_channel, 1)
        conv_layer = nn.Conv1d(input_channel, output_channel, 25, 1, 12).to(outs.device, dtype=outs.dtype)

        outs = conv_layer(outs)

        # outs = outs.view(pre_shape)

        # if data_format == 'NHWC':
        #     outs = outs.permute(0, 2, 3, 1).contiguous()
        outs = outs.view(b,output_channel,_,h,w)
        # outs = rearrange(outs, 'b (a1 h a2 w) c -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        return outs
    ...
    def forward(self, buffer):
        ...
        buffer = self.rev_conv2d(outs=buffer_tmp,input_channel=180,output_channel=60,kernel_size=3) + buffer
        return buffer
