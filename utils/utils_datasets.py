...
class TrainSetDataLoader(Dataset):
    def __init__(self, args):
        ...
        self.file_list = self.data_list
        # self.file_list = []
        # for data_name in self.data_list:
        #     file_path = os.path.join(args.path_for_train, data_name)
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(file_name[0], 'r') as hf:
            Lr_SAI_y = np.array(hf.get('data')) # Lr_SAI_y
            Hr_SAI_y = np.array(hf.get('label')) # Hr_SAI_y
...
class TestSetDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        file_name = [self.dataset_dir + self.file_list[index]]
        with h5py.File(file_name[0], 'r') as hf:
            Lr_SAI_y = np.array(hf.get('data'))
            Hr_SAI_y = np.array(hf.get('label'))
